...
def cube_imp(weights, indices_rs, pts, cnts, is_train, n_samples, **kwargs):
    ...
    pts = torch.cat([pts, pts_rs + cnts[:,:, None]], 2)
    # print(f"pts.shape={pts.shape}, cnts.shape={cnts.shape}")
...
def cube_sample_pdf(pts, cnts, weights, indices_rs, N_samples, is_train):
    ...
    # print(f"rs_mid.shape={rs_mid.shape}")
    ...
    bins_g = torch.gather(rs_mid.unsqueeze(2).expand(matched_shape), 3, inds_g)
